		...
		const code = `[${
			routes
				...
				.map(route => {
					...
					return `{ pattern: ${route.pattern}, params: match => (${params}), load: () => import(/* webpackChunkName: "${route.id}" */ '${src}${path.sep}${route.file}') }`
				})
				.join(', ')
		}]`;
		...
		const imports = routes
			.map(route => {
				return route.type === 'page' ?
					`import ${route.id} from '${src}${path.sep}${route.file}';` :
					`import * as ${route.id} from '${src}${path.sep}${route.file}';`;
			})
			.join('\n');
