...
describe('assetsPrefix simple publicPath', function () {
	this.timeout(10000);
	...
	it('simple publicPath', async () => {
		...
		assert.ok(
			await r.page.evaluate(() => {
				...
				return /^https:\/\/127.0.0.1:9999\/client\//.test(src);
			}),
			'main.js path error'
		);
		assert.ok(
			await r.page.evaluate(() => {
				return [...document.querySelectorAll('link')]
					...
					.every((link) => /^https:\/\/127.0.0.1:9999\/client\//.test(link.href));
			}),
			'css path error'
		);
		assert.ok(
			validateLink(res.headers().link, new RegExp('^<https://127.0.0.1:9999/client/')),
