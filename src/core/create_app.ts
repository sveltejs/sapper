...
import { CssBuildInfo } from './create_compilers/interfaces';
...
export function create_serviceworker_manifest({ manifest_data, output, js_files, css_files, static_files }: {
	manifest_data: ManifestData;
	output: string;
	js_files: string[];
	css_files: CssBuildInfo;
	static_files: string;
}) {
	...
	let client_files = [...js_files];
	if (css_files.main) {
		client_files.push(css_files.main);
	}
	for (const chunks of Object.values(css_files.chunks)) {
		client_files.push(...chunks);
	}
	client_files = client_files.map(file => `client/${file}`);
