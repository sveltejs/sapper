...
export function create_app({
	...
	const client_manifest = generate_client_manifest(manifest_data, path_to_routes, bundler, dev, dev_port, spa);
	const server_manifest = generate_server_manifest(manifest_data, path_to_routes, cwd, src, dest, dev, spa);
...
export function create_serviceworker_manifest({ manifest_data, output, client_files, static_files, spa }: {
	manifest_data: ManifestData;
	output: string;
	client_files: string[];
	static_files: string;
	...
	let files: string[] = [spa ? 'index.html' : 'service-worker-index.html'];
...
function generate_client_manifest(
	...
	let route_list = manifest_data.pages.map(page => {
		...
		let route = get_route_factory(component.file, part.params.length > 0);
...
function get_route_factory(file: string, has_params: boolean) {
