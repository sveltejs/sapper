...
function generate_app(manifest_data: ManifestData, path_to_routes: string) {
	...
	while (l-- > 1) {
		pyramid = `
			<svelte:component this={level${l}.component} segment={level${l + 1} && level${l + 1}.segment} {...level${l}.props}>
				{#if level${l + 1}}
					${pyramid.replace(/\n/g, '\n\t\t\t\t\t')}
				{/if}
			</svelte:component>
	...
	return `
		...
		<script>
			...
			export let status;
			export let level0;
			${levels.map(l => `export let level${l};`).join('\n\t\t\t')}

			setContext(CONTEXT_KEY, session);
		</script>

		<Layout segment={level1.segment} {...level0.props}>
			{#if error}
				<Error {error} {status}/>
			{:else}
				${pyramid.replace(/\n/g, '\n\t\t\t\t')}
