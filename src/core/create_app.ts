...
export function create_app({
	...
	hashbang = !ssr && hashbang;

	const client_manifest = generate_client_manifest(manifest_data, path_to_routes, bundler, dev, dev_port, ssr, hashbang);
	const server_manifest = generate_server_manifest(manifest_data, path_to_routes, cwd, src, dest, dev, ssr, template);
...
export function create_serviceworker_manifest({ manifest_data, output, client_files, static_files, ssr }: {
	manifest_data: ManifestData;
	output: string;
	client_files: string[];
	static_files: string;
	...
	let files: string[] = [(ssr ? 'service-worker-index.html' : 'index.html')];
