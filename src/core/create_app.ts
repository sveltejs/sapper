...
export function create_app({
	...
	const server_manifest = generate_server_manifest(manifest_data, path_to_routes, cwd, root, src, dest, dev, export_build);
...
function generate_server_manifest(
	...
	const root_path = export_build ? cwd : root;
	const build_dir = posixify(path.normalize(path.relative(root_path, dest)));
	const src_dir = posixify(path.normalize(path.relative(root_path, src)));
