...
async function _export({
	...
	const saved = new Set();
	// const seenQueue: URL[] = [];
	// const readQueue: Array<Promise<{ promise: Promise<string|Response>, isResolved: boolean }>> = [];
	// const writeQueue = [];
	...
	function save(url: string, status: number, type: string, body: string) {
		...
		return new Promise((res) => {
			fs.writeFile(export_file, body, res);
		});
	}
  ...
  //   	.then(v => (v === t)? "pending" : "fulfilled", () => "rejected");
	...
	// 	while (readQueue.length < concurrent && seenQueue.length) {
	// 		const url = seenQueue.shift();
	// 		const fetching = new Promise((resolve, reject) => {
	// 			fetchUrl(url).then((ret) => {
	// 				resolve(ret);
	// 			});
	// 		});
	// 		readQueue.push(fetching);
	// 	}
	// }

	// function enqueueWrite() {
	// 	let resolved = readQueue.find(reading => reading.isResolved);
	// 	while (writeQueue.length < concurrent && resolved) {
	// 		writeQueue.push();
	// 		resolved = readQueue.find(reading => reading.isResolved);
	// 	}
	// }

	// function processQueues() {
	// 	if (readQueue.length < concurrent && seenQueue.length > 0) {
	// 		readQueue.push(fetchUrl(seenQueue.shift()));
	// 	}

	// 	Promise.race()

	// 	if (writeQueue.length < concurrent) {
	// 		const resolved = readQueue.find(reading => reading.isResolved !== undefined);
	...
	async function handle(url: URL) {
		...
		const r = await q.add(async () => {
			...
			return save(pathname, r.status, type, body);
		});
