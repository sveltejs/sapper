...
function middleware(_a) {
    ...
    var client_assets = JSON.parse(fs.readFileSync(path.join(output, 'client_assets.json'), 'utf-8'));
...
function get_route_handler(chunks, App, routes, store_getter) {
    ...
    function handle_route(route, req, res) {
        ...
        var handlers = route.handlers[Symbol.iterator]();
        function next() {
            try {
                var _a = handlers.next(), handler = _a.value, done = _a.done;
                if (done) {
                    handle_error(req, res, 404, 'Not found');
                    return;
                }
                var mod_1 = handler.module;
                if (handler.type === 'page') {
                    ...
                    var store_1 = store_getter ? store_getter(req) : null;
                    var props_1 = { params: req.params, query: req.query, path: req.path };
                    var redirect_1;
                    var error_1;
                    Promise.resolve(mod_1.preload ? mod_1.preload.call({
                        ...
                        if (redirect_1) {
                            ...
                            res.setHeader('Location', req.baseUrl + "/" + redirect_1.location);
                            res.end();
                            return;
                        }
                        if (error_1) {
                        ...
                        var serialized = {
                            preloaded: mod_1.preload && try_serialize(preloaded),
                            store: store_1 && try_serialize(store_1.get())
                        };
                        Object.assign(props_1, preloaded);
                        var _a = App.render({ Page: mod_1, props: props_1 }, {
                            store: store_1
                    ...
                    var handle_method = mod_1[method_export];
                    if (handle_method) {
    ...
    function handle_error(req, res, statusCode, message) {
        ...
        var route = not_found
            ? not_found_route
            : error_route;
        function render_page(_a) {
            var head = _a.head, css = _a.css, html = _a.html;
            var page = template()
                .replace('%sapper.base%', "<base href=\"" + req.baseUrl + "/\">")
        ...
        if (route) {
            var handlers_1 = route.handlers[Symbol.iterator]();
            function next() {
                var _a = handlers_1.next(), handler = _a.value, done = _a.done;
                if (done) {
                    handle_notfound();
                }
                    ...
                    var handle_method = mod[method_export];
                    if (handle_method) {
    ...
    return function find_route(req, res) {
        for (var _i = 0, routes_1 = routes; _i < routes_1.length; _i++) {
            ...
            if (!route.error && route.pattern.test(req.path))
