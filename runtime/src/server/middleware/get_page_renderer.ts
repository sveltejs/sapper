...
export function get_page_renderer(
	manifest: Manifest,
	...
	return async function render_page(
		page: ManifestPage,
		req: SapperRequest,
		res: SapperResponse,
		error: any = null
	) {
		const statusCode = error ? (error.status ?? error.statusCode ?? 500) : 200;
		...
		const preload_context = {
			...
			error: (statusCode: number, error: any) => {
				preload_error = http_error(statusCode, error);
			},
		...
		try {
			...
			if (error) {
				// encountering a new error while rendering an error page, bounce
				throw err;
			}

			preload_error = http_error(500, err);
		...
		if (error instanceof Error) {
			...
			contextError = new Error(error);
			contextError.name = 'PreloadError';
		}
		...
		let script = `__SAPPER__={${[
			error && `error:${serialized.error},status:${statusCode}`,
			`baseUrl:"${req.baseUrl}"`,
...
function http_error(statusCode: number, err: any) {
	const http_err: HttpError = err instanceof Error ? err : new Error(err);
	http_err.statusCode = statusCode;
	return http_err;
}
